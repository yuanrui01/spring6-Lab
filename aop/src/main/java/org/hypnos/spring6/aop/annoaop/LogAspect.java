package org.hypnos.spring6.aop.annoaop;
...
public class LogAspect {
    ...
    // 返回 @AfterReturning
    @AfterReturning(value = "execution(* org.hypnos.spring6.aop.annoaop.CalculatorImpl.*(..))",returning = "result")
    ...
    //目标方法出现异常，这个通知执行
    @AfterThrowing(value = "execution(* org.hypnos.spring6.aop.annoaop.CalculatorImpl.*(..))",throwing = "ex")
    ...
    // 环绕 @Around()
/*    @Around("execution(* org.hypnos.spring6.aop.annoaop.CalculatorImpl.*(..))")
    ...
    //重用切入点表达式
    @Pointcut(value = "execution(* org.hypnos.spring6.aop.annoaop.CalculatorImpl.*(..))")
